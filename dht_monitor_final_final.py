...
# Korjaa tiedostopolut .exe-tiedostolle
def get_app_dir():
    """Palauttaa oikean hakemiston riippumatta siitä, onko kyseessä .py vai .exe."""
    if getattr(sys, 'frozen', False):
        # Jos ohjelma on käännetty .exe-tiedostoksi
        return os.path.dirname(sys.executable)
    else:
        # Jos ohjelma ajetaan .py-tiedostona
        return os.path.dirname(os.path.abspath(__file__))

APP_DIR = get_app_dir()
DATABASE_PATH = os.path.join(APP_DIR, "mittaukset.sqlite3")
ESP32_URL = "http://192.168.4.1/data"
POLL_INTERVAL_SECONDS = 1800 # 1800 = 30 min
MAX_POINTS = 900
...
class ScrollableFrame(ttk.Frame):
    """Kehys, joka sisältää vierityspalkilla varustetun sisäkehyksen."""
    
    def __init__(self, parent, **kwargs):
        super().__init__(parent, **kwargs)
        
        # Luo canvas ja vierityspalkki
        self.canvas = tk.Canvas(self, highlightthickness=0)
        self.scrollbar = ttk.Scrollbar(self, orient="vertical", command=self.canvas.yview)
        self.canvas.configure(yscrollcommand=self.scrollbar.set)
        
        # Sisäkehys, johon kaikki sisältö tulee
        self.inner = ttk.Frame(self.canvas, style="App.TFrame")
        
        # Luo ikkuna canvasille
        self.window_id = self.canvas.create_window((0, 0), window=self.inner, anchor="nw")
        
        # Pakkaa canvas ja scrollbar
        self.scrollbar.pack(side="right", fill="y")
        self.canvas.pack(side="left", fill="both", expand=True)
        
        # Päivitä scrollbar, kun sisäkehyksen koko muuttuu
        self.inner.bind("<Configure>", self._on_frame_configure)
        self.canvas.bind("<Configure>", self._on_canvas_configure)
        
        # Salli hiiren rullaus
        self.bind_mousewheel()
    
    def _on_frame_configure(self, event):
        """Päivitä scrollbarin aluetta, kun sisäkehyksen koko muuttuu."""
        self.canvas.configure(scrollregion=self.canvas.bbox("all"))
    
    def _on_canvas_configure(self, event):
        """Muuta canvas-ikkunan leveyttä, kun canvasin koko muuttuu."""
        self.canvas.itemconfig(self.window_id, width=event.width)
    
    def bind_mousewheel(self):
        """Lisää hiiren rullaus tuki."""
    ...
    def unbind_mousewheel(self):
        """Poista hiiren rullaus sidokset (kutsutaan kun ikkuna suljetaan)."""
...
class App(tk.Tk):
    ...
    def build(self):
        # Pääkehys, joka sisältää vierityspalkin
        main_container = ttk.Frame(self, style="App.TFrame")
        main_container.pack(fill="both", expand=True)
        
        # Luodaan vieritettävä kehys
        self.scrollable = ScrollableFrame(main_container, style="App.TFrame")
        self.scrollable.pack(fill="both", expand=True)
        
        # Sisäkehys, johon kaikki sisältö tulee
        root = self.scrollable.inner
    ...
    def destroy(self):
        # Poista hiiren rullaustapahtumat
        if hasattr(self, "scrollable"):
            self.scrollable.unbind_mousewheel()
